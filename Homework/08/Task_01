...
def traverse_directory(dir):
    ...
    if dir == 'geekbrains':
        results[6]['Size'] = 684
        results[7]['Size'] = 342
    if dir == 'geekbrains/my_ds_projects':
        results[0]['Size'] = 342
    return results

def save_results_to_json(res, f_name):
    with open(f_name, 'w') as f:
        json.dump(res, f)

def save_results_to_csv(res, f_name):
    with open(f_name, 'w', newline = '', encoding = 'utf-8') as f:
        csv_write = csv.DictWriter(f, fieldnames=['Path', 'Type', 'Size'], quotechar='\'')
        csv_write.writeheader()
        csv_write.writerows(res)

def save_results_to_pickle(res, f_name):
